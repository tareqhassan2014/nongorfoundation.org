...
const EssentialsTried = () => {
    const filterEssentialsTriad = essentialsTried.filter(data => data.image)
    return (
        <div className='py-8 lg:py-16 space-y-10 lg:space-y-20 text-center max-w-7xl mx-auto px-2 lg:px-0'>
            ...
            {
                filterEssentialsTriad.map(data => <div key={data.id}>
                    <h1 className='text-[#424242] text-5xl font-serif font-semibold mb-5'>{data.name}</h1>
                    {
                        data.image
                        &&
                        <Image src={data.image} alt='medical_help' height={450} width={950} className='max-w-60 md:max-w-2xl lg:max-w-5xl mx-auto border object-cover'></Image>
                    }
...
export default EssentialsTried;