...
const EssentialsTriad = () => {
  const filterCourseOfAction = essentialsTried.filter(
    (data) => data.courseOfAction
  );
  return (
    <div className="mt-36 max-w-6xl mx-auto font-serif px-1 lg:px-0">
      ...
      <div className="mt-14 flex gap-10 flex-col lg:flex-row ">
        ...
        <Image src="/home/Essentials-triad-bannar.avif" alt="Banner image" height={650} width={650} className="rounded-lg" />
        <div>
          {filterCourseOfAction.map((data) => (
            <div
              key={data.id}
              className="flex flex-col justify-between  p-5 rounded-lg h-full shadow-md border-t-4 border-blue-950"
            >
              ...
              <Link
                href={"/pages/essentials-triad"}
                className="pl-3 underline text-red-500 font-semibold text-sm flex justify-end"
              >
                Learn more
              </Link>
            </div>
