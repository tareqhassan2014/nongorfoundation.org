...
const MedicalCamp = () => {
    const filterCourseOfAction = freeMedicalCamp.filter(data => data.courseOfAction)
    return (
                ...
                <div>
                    {
                        filterCourseOfAction.map(data => <div key={data.id} className="flex flex-col justify-between  p-5 rounded-lg h-full shadow-md border-t-4 border-blue-950">
                            ...
                            <Link href={"/pages/medical-help"} className="pl-3 underline text-red-500 font-semibold text-sm flex justify-end">Learn more</Link>
                        </div>)
                    }
                </div>
            </div>
