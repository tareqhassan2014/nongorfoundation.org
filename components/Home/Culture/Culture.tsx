...
const Culture = () => {
    const filterCourseOfAction = culture.filter(data => data.courseOfAction)

    return (
        <div className="mt-36 max-w-6xl mx-auto font-serif px-1 lg:px-0">
            <h1 className="text-[#424242] text-center text-5xl font-serif font-semibold mb-4">Culture</h1>
            ...
            <div className="mt-14 flex gap-10 flex-col lg:flex-row-reverse ">
                <Image src="/home/culture-Banner.jpg" alt="Banner image" height={350} width={650} className="object-cover rounded-lg" />
                <div>
                    {
                        filterCourseOfAction.map(data => <div key={data.id} className="flex flex-col justify-between  p-5 rounded-lg h-full shadow-md border-t-4 border-blue-950">
                            ...
                            <Link href={"/pages/culture"} className="pl-3 underline text-red-500 font-semibold text-sm flex justify-end">Learn more</Link>
                        </div>)
                    }
                </div>
            </div>
